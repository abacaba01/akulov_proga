...
def house(x, y):
    ...
    '''
    функция рисует дом, где (x,y) - координаты его центра
    '''
...
'''
далее идут функции облаков, каждая из которых создает облако определенного цвета
(x, y) - кординаты левого верхнего кгла пряиоугольника, в котором расположено облако
z - ширина этого прямоугольника
m - высота этого прямоугольника
'''
...
def ghost(x, y, t, m):
    ...
    '''
    функция рисует призрака, с центром головы в точках (x, y)
    его размеры регулируются переменной t
    отвечает за направление взгляда призрака m
    '''
    ...
    polygon(screen, (179, 179, 179), [(x - t, y), ((x - 4 * t), (y + 5 * t)), ((x - 3 * t), (y + 4 * t)), ((x - 2 * t),
            (y + 5 * t)), ((x - t), (y + 4 * t)), (x, (y + 5 * t)), ((x + t), (y + 4 * t)), ((x + 2 * t), (y + 5 * t)),
